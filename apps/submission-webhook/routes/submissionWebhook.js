...
} = require("../middleware/verifySubmissionResultData");
...
async function updateTestCaseAndSubmission(token, testCaseData) {
  const testCase = await testCaseDAO.updateByJudge0TrackingId(
    token,
    testCaseData,
  );
...
router.route("/").put(checkSubmissionResultData, async (req, res) => {
  try {
...
const getSubmissionStatus = (testCases) => {
  return testCases.every((testCase) => testCase.status === "AC")
    ? "Accepted"
    : "Rejected";
};
